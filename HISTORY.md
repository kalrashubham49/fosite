...
The [`fosite/handler/oauth2.JWTStrategy`](https://github.com/kalrashubham49/fosite/blob/master/handler/oauth2/strategy.go) interface changed as a context
...
The [`OpenIDConnectRequestValidator.ValidatePrompt`](https://github.com/kalrashubham49/fosite/blob/master/handler/openid/validator.go)
...
The [`Hasher`](https://github.com/kalrashubham49/fosite/blob/master/hash.go) interface
...
The [`JWTStrategy`](https://github.com/kalrashubham49/fosite/blob/master/token/jwt/jwt.go) interface
changed as a context parameter was added to its method signatures:
...
- `github.com/kalrashubham49/fosite/token/jwt.JWTClaims.Audience` is no longer a `string`, but a string slice `[]string`.
- `github.com/kalrashubham49/fosite/handler/openid.IDTokenClaims` is no longer a `string`, but a string slice `[]string`.

### `AuthorizeCodeStorage`

This improves security as, in the event of an authorization code being leaked, all associated tokens are revoked. To
implement this feature, a breaking change had to be introduced. The `github.com/kalrashubham49/fosite/handler/oauth2.AuthorizeCodeStorage`
...
- The error `github.com/kalrashubham49/fosite/handler/oauth2.ErrInvalidatedAuthorizeCode` has been added.
...
[`statusCode` was changed to `status_code`](https://github.com/kalrashubham49/fosite/pull/242/files#diff-dd25e0e0a594c3f3592c1c717039b85eR221).
...
A new [scope strategy](https://github.com/kalrashubham49/fosite/pull/187) was introduced called `WildcardScopeStrategy`. This strategy is now the default when using
...
The [`ClientManager`](https://github.com/kalrashubham49/fosite/blob/master/client_manager.go) interface
...
The [OAuth2Provider](https://github.com/kalrashubham49/fosite/blob/master/oauth2.go) interface changed,
...
* Move the repo from `github.com/ory-am/fosite` to `github.com/kalrashubham49/fosite`
